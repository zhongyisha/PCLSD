#ifndef PCLSD_PCLSD_H_
#define PCLSD_PCLSD_H_
...
struct PCLSDParams {
  ...
  // The threshold over the validation criteria. For PCLSD, it is the gradient angular error in pixels.
 ...
 *     PCLSD: Enhanced Line SEgment Drawing. arXiv preprint arXiv:2108.03144.
 *
...
class PCLSD {
 public:
  // Constructor
  explicit PCLSD(const PCLSDParams &params = PCLSDParams());
  ...
  computeGradients(const cv::Mat &srcImg, short gradientTh, cv::Mat& edgeMap);
  ...
  void drawPoint(cv::Mat img);

  bool next(int& xSeed, int& ySeed);

  void optImg(cv::Mat& edgeMap, cv::Mat& gradientMap);
