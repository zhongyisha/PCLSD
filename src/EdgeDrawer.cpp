...
void EdgeDrawer::init(const LineDetectionExtraInfoPtr &gradientInfo, cv::Mat &edgeImage) {
  ...
  //ÐÂ¼ÓµÄ
  assert(!gradientInfo->oriImg.empty());
  assert(gradientInfo->oriImg.type() == CV_8UC1);
  assert(gradientInfo->oriImg.size() == gradientInfo->gImg.size());
  pOriImg = gradientInfo->oriImg.ptr<uint8_t>();
  //-------------
